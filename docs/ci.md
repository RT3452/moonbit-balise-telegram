...
CI uses `moon check --target all` to compile every backend, then runs the default and native test targets. JavaScript execution additionally requires a Node.js runtime; it can be enabled in environments that provide Node.
